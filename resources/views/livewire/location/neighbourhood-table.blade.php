            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($data as $item)
                    <tr class="hover:bg-gray-50 cursor-pointer relative {{ $selectedRowId == $item->id ? 'bg-blue-50' : '' }}"
                        ...
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{{ $item->district->city->name }}</td>
                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{{ $item->district->name }}</td>
