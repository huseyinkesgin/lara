<div class="relative" wire:key="neighborhood-table-{{ $page ?? '1' }}">
    <div class="flex justify-between">
        <div class="bg-white p-2 border-b flex items-center gap-2">
            <x-widget3 icon="fa-home" title="Mahalle Sayısı:">
                {{ $neighborhoodCount }}
            </x-widget3>
        </div>
    </div>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @foreach ($data as $item)
                <tr class="hover:bg-gray-50 cursor-pointer relative {{ $selectedRowId == $item->id ? 'bg-blue-50' : '' }}"
                    ...
                    <x-td>{{ $item->code }}</x-td>
                    <x-td>{{ $item->district->town->city->name }}</x-td>
                    <x-td>{{ $item->district->town->name }}</x-td>
