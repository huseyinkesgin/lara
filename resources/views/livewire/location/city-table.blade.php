<div class="relative" wire:key="city-table-{{ $page ?? '1' }}">
    <div class="flex justify-between">
        <div class="bg-white p-2 border-b flex items-center gap-2">
            ...
            <x-widget1   icon="fa-city" title="Şehir Sayısı:">
                {{ $cityCount }}
            </x-widget1>
            <x-widget2 icon="fa-building" title="İlçe Sayısı:">
                {{ $townCount }}
            </x-widget2>
    ...
    <x-table>
        <thead class="bg-gray-50">
            <tr>
                <x-th>Kod</x-th>
                <x-th>Il Adı</x-th>
                <x-th>İlçe Sayısı</x-th>
                <x-th>Semt Sayısı</x-th>
                <x-th>Durum</x-th>
                <x-th>O.Tarihi</x-th>
                <x-th>G.Tarihi</x-th>
            </tr>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @foreach ($data as $item)
                <tr class="hover:bg-gray-50 cursor-pointer relative {{ $selectedRowId == $item->id ? 'bg-blue-50' : '' }}"
                    ...
                    <x-td>{{ $item->code }}</x-td>
                    <x-td>{{ $item->name }}</x-td>
                    <x-td>{{ $item->towns->count() }}</x-td>
                    <x-td>{{ $item->districts->count() }}</x-td>
                    <x-td>
                        <span
